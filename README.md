...
- **Fixes:** the newest message is no longer cut off at the bottom of
  ...
  and a gateway connection that dies without saying so (a sleep, a
  network change) is noticed within 15 s of an unanswered heartbeat and
  resumed, where before the client sat "Connected" with no messages,
  typing or presence arriving until the socket itself gave up.
